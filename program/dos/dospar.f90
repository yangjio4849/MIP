...
program dospar 
  ...
  logical yon,splitd                  ! yes or no  ,  d eletrons split to t2g & eg
...
out:do while(.TRUE.)
  ...
  if(nomeaning=="LSORBIT") then
    backspace(38)
    read(38,*) nomeaning , nomeaning, SOC
    exit out
  endif
end do out
...
! print the heading
if (nspin==1) then
  write(38,"(A10,2A17)")adjustl("E (eV)"),adjustl("DOS (a.u.)"),adjustl("intDOS")
else
  write(38,"(A10,4A17)")adjustl("E (eV)"),adjustl("DOS_up (a.u.)"),adjustl("DOS_dn (a.u.)"),adjustl("intDOS_up"),adjustl("intDOS_dn") 
endif
  ...
  do ik=1,nk
    ...
    do ispin=1,nspin
      if (.not.SOC) then
        ...
        dos(ispin)=dos(ispin)+dostemp(ispin)/2*kwpt(ik)
        intdos(ispin)=intdos(ispin)+intdostemp(ispin)/2*kwpt(ik)
      endif
    enddo
  enddo
...
!print the heading
  if (nspin==1) then
    if (ncol==4) then
      write(38,"(A12,4A17)")adjustl("E (eV)"),adjustl("s (a.u.)"),adjustl("p (a.u.)"),adjustl("d (a.u.)"),adjustl("tot (a.u.)")
    else
      write(38,"(A12,5A17)")adjustl("E (eV)"),adjustl("s (a.u.)"),adjustl("p (a.u.)"),adjustl("d (a.u.)"),adjustl("f (a.u.)"),adjustl("tot (a.u.)")
    endif
  else
    if (ncol==4) then
      write(38,"(A12,8A17)")adjustl("E (eV)"),adjustl("s_up (a.u.)"),adjustl("p_up (a.u.)"),adjustl("d_up (a.u.)"),adjustl("tot_up (a.u.)"),adjustl("s_dn (a.u.)"),adjustl("p_dn (a.u.)"),adjustl("d_dn (a.u.)"),adjustl("tot_dn (a.u.)")
    else
      write(38,"(A12,10A17)")adjustl("E (eV)"),adjustl("s_up (a.u.)"),adjustl("p_up (a.u.)"),adjustl("d_up (a.u.)"),adjustl("f_up (a.u.)"),adjustl("tot_up (a.u.)"),adjustl("s_dn (a.u.)"),adjustl("p_dn (a.u.)"),adjustl("d_dn (a.u.)"),adjustl("f_dn (a.u.)"),adjustl("tot_dn (a.u.)")
    endif
  endif
      ...
      do icol=1,ncol
        do ik=1,nk
          ...
          if (.not.SOC) then
            ...
            dos(icol,ispin)=dos(icol,ispin)+dostemp/2*kwpt(ik)
          endif
        enddo     ! k-points
      enddo       ! column
    enddo         ! spin
